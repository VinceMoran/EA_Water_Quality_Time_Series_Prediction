...
def set_random_seed(seed: int=24, deterministic: bool=True):
    """
    Set random seeds for reproducible results in Python, NumPy, and PyTorch.

    Args:
      - seed (int): The random seed value to use for all PNRGs.
      - deterministic (bool): Makes PyTorch operations deterministic if True (may reduce performance slightly but allows full reproducibility).

    Returns:
      - None.

    Side effects:
      - Sets random seeds to specified value when initialising all PNRGs in Python, NumPy, and PyTorch.
      - Makes all PyTorch operations deterministic if `deterministic=True` (default).
    """
    ...
    torch.cuda.manual_seed_all(seed) # Pytorch (GPUs)

    # Make PyTorch operations deterministic if `deterministic=True`
    if deterministic:
...
def compute_performance_metrics(y_obs: np.ndarray,
  ...
  mse = mean_squared_error(y_true=y_obs,
                           y_pred=y_pred)
...
def rolling_median_detect(dataframe: pd.DataFrame,
                          variable_name: str,
                          window: int = 6,
                          threshold: float = 3,
                          min_periods: int = None):
    """
    Takes a variable as an input and calculates the rolling median and median
    absolute deviation. Creates a new Pandas Series that uses Boolean values to
    indicate flagged anomalies that fall outside a specified threshold.

    Args:
      - dataframe (pd.DataFrame): A pandas DataFrame to be investigated.
      - variable_name (str): The name of the variable, used to index a DataFrame.
      - window (int): The size of the rolling window used to calculate the median and median absolute deviation.
      - threshold (float): The number of median absolute deviations used to flag anomalies.

    Returns:
      - anomalies (pd.Series): A Boolean data series that flags anomalies.
    """
    # Calculate the rolling median and median absolute deviation
    rolling_median = dataframe[variable_name].rolling(window=window, min_periods=min_periods).median()
    rolling_mad = dataframe[variable_name].rolling(window=window, min_periods=min_periods).apply(
        lambda x: np.median(np.abs(x - np.median(x)))
        )
    # Tune to match standard z-score outlier detection assuming Gaussian noise
    scaled_rolling_mad = 1.4826 * rolling_mad

    # Flag anomalies above the threshold
    anomalies = (dataframe[variable_name] > rolling_median + threshold * scaled_rolling_mad) | (dataframe[variable_name] < rolling_median - threshold * scaled_rolling_mad)
    anomalies = anomalies.fillna(False) # Flag NaN values at the start of the series as False

    # Return a Boolean data series that flags anomalies
    return anomalies

def rolling_mean_detect(dataframe: pd.DataFrame,
                        variable_name: str,
                        window: int,
                        threshold: float):
    """
    Takes a variable as an input and calculates the rolling mean and standard
    deviation. Creates a new Pandas Series that uses Boolean values to indicate
    flagged anomalies that fall outside a specified threshold. This function was
    included and tested for completeness of analyses, however median absolute
    deviation was more suitable for anomaly detection in most datasets within
    this project.

    Args:
      - dataframe (pd.DataFrame): A pandas DataFrame to be investigated.
      - variable (str): The name of the variable, used to index a DataFrame.
      - window (int): The size of the rolling window used to calculate the mean and standard deviation.
      - threshold (float): The number of standard deviations used to flag anomalies.
    """

    # Calculate the rolling mean and standard deviation
    rolling_mean = dataframe[variable_name].rolling(window=window).mean()
    rolling_std = dataframe[variable_name].rolling(window=window).std()

    # Flag anomalies above the treshold
    anomalies = (dataframe[variable_name] > rolling_mean + threshold * rolling_std) | (dataframe[variable_name] < rolling_mean - threshold * rolling_std)
    anomalies = anomalies.fillna(False) # Flag NaN values at the start of the series as False
...
class TimeSeries(DataModule):
    ...
    def __init__(self,
        ...
        self.y_train_tensor = torch.tensor(y_train, dtype=torch.float32)[num_steps:].reshape(-1, 1)
        self.X_val_tensor = torch.tensor(X_val, dtype=torch.float32).unfold(0, num_steps, 1)[:-1].permute(0, 2, 1)
        self.y_val_tensor = torch.tensor(y_val, dtype=torch.float32)[num_steps:].reshape(-1, 1)
        self.X_test_tensor = torch.tensor(X_test, dtype=torch.float32).unfold(0, num_steps, 1)[:-1].permute(0, 2, 1)
        self.y_test_tensor = torch.tensor(y_test, dtype=torch.float32)[num_steps:].reshape(-1, 1)
...
class Trainer(HyperParameters):
    ...
    def __init__(self, max_epochs, num_gpus=0, gradient_clip_val=0, early_stopping=True, patience=5, min_delta=0.0, restore_best_params=True):
        """
        ...
        Args:
          ...
          - early_stopping (bool): Enables early stopping to prevent overfitting once validation loss stops decreasing.
          - patience (int): Stops training after this number of epochs after validation loss stops decreasing (if `early_stopping=True).
          - min_delta (float): The minimum reduction in validation loss required to avoid early stopping (if `early_stopping=True).
          - restore_best_params (bool): Reload and use the model parameters that resulted in the lowest validation loss.
        ...
        # Initialise the early stopping state
        self.best_val_loss = float("inf") # sets the best validation loss to infinity prior to training
        self.best_model_state = None # sets best model state to `None` prior to training
        self.wait = 0 # sets the counter for number of epochs without validation improvement to zero prior to training
    ...
    def fit(self, model, data):
        ...
        for self.epoch in range(self.max_epochs):
            ...
            self.fit_epoch()
            # Check for early stopping criterion if early stopping is enabled and a validation DataLoader is present
            if self.early_stopping and self.val_dataloader is not None:
                # Compute the average validation loss over all batches
                val_loss = self.evaluate_validation_loss()
                # Check whether the current validation loss is less than the best validation loss (within `min_delta` improvement criterion)
                if val_loss < self.best_val_loss - self.min_delta:
                    # Set the current validation loss as the best validation loss if within improvement criterion
                    self.best_val_loss = val_loss
                    # Save the model state dictionary that resulted in the best validation loss
                    self.best_model_state = {k: v.cpu().clone() for k, v in self.model.state_dict().items()}
                    # Set the patience counter to zero
                    self.wait = 0
                # Triggered if current validation loss is not less than best validation loss
                else:
                    # Add 1 epoch to the patience counter
                    self.wait += 1
                    # Document that validation loss has not improved for {wait/patience} epochs
                    print(f"[EarlyStopping] No improvement for {self.wait}/{self.patience} epochs.")
                # Stops training the model once the number of epochs without validation loss improvement exceeds the patience criterion
                if self.wait >= self.patience:
                    # Document that the early stopping criterion has been triggered and that the model will stop training to prevent overfitting
                    print(f"[EarlyStopping] Stopping early at epoch {self.epoch+1}. Best val loss: {self.best_val_loss:.6f}")
                    # Checks whether the user intends to store the parameters that resulted in the lowest validation loss
                    if self.restore_best_params and self.best_model_state is not None:
                        # Loads the model parameters that resulted in the lowest validation loss
                        self.model.load_state_dict(self.best_model_state)
                    # Stops model training early once early stopping criterion is triggered
                    break
    ...
    def fit_epoch(self):
        ...
        for batch in self.train_dataloader:
            ...
            # Zero the gradients stored in the optimiser
            self.optim.zero_grad()
            # Perform backpropagation of gradients
            loss.backward()
            if self.gradient_clip_val > 0:
                # Clips gradients to below the maximum value if gradient clipping is enabled to ensure numerical stability
                self.clip_gradients(self.gradient_clip_val, self.model)
            # Step the optimiser to update model parameters
            self.optim.step()
            # Add 1 to the counter to index the next training batch
            self.train_batch_idx += 1
        # Check whether a validation DataLoader is present
        if self.val_dataloader is None:
            return
        # Set the model to evaluation mode if a validation DataLoader is present
        self.model.eval()
        # Loop through batches in the validation DataLoader
        for batch in self.val_dataloader:
            # Turn off gradient tracking to speed up computations
            with torch.no_grad():
                # Complete a validation step on the current batch
                self.model.validation_step(self.prepare_batch(batch))
            # Add 1 to the counter to index the next validation batch
            self.val_batch_idx += 1

    def evaluate_validation_loss(self):
        """
        Computes the  average validation loss over all batches.
        ...
        self.model.eval()
        # Set the initial total loss and batch count to zero
        total_loss = 0
        count = 0
        # Loop through the batches in the validation DataLoader
        for batch in self.val_dataloader:
            # Turns off gradient tracking to speed up computations
            with torch.no_grad():
                # Prepare the features and targets in the batch
                X, y = self.prepare_batch(batch)
                # Perform the model forward pass on the inputs from the current batch
                y_hat = self.model(X)
                # Compute the loss from current predictions and targets from the batch
                loss = self.model.loss(y_hat, y)
                # Add the loss from the batch to the total loss
                total_loss += loss.item()
                # Add 1 to the counter for averaging loss across batches
                count += 1
...
class RNNWQ(Regressor):
    ...
    def predict(self, X, seq_length=72, batch_size=1024, device="cuda" if torch.cuda.is_available() else "cpu"):
        """
...
# Define the base class for training within a hyperparamter optimisation workflow
class HPOTrainer(Trainer):
    """
    Base class for training a model within a hyperparameter optimisation
    workflow such as grid search or asynchronous successive halving.

    Inherits from Trainer.

    Modified from Source: https://raw.githubusercontent.com/d2l-ai/d2l-en/refs/heads/master/d2l/torch.py
    """
    # Define a method for computing the validation error of the current model
    def validation_error(self):
        """
        Defines a method that operates on the Trainer's internal state to
        compute the validation error of the current model.

        Args:
          - None.

        Returns:
          - The validation error of the current model.
        ...
        # Set the model to evaluation mode
        self.model.eval()
        # Intialise a variable to hold the sum of mean absolute error across validation batches
        mae = 0
        # Initialise a counter for how many validation batches have been processed to compute average mean absolute error
        val_batch_idx = 0
        # Loop through batches in the validation dataset
        for batch in self.val_dataloader:
            # Turn off gradient tracking for faster inference
            with torch.no_grad():
                # Unpack the batch into an input feature tensor (X) and an output target tensor (y)
                x, y = self.prepare_batch(batch)
                # Compute predictions by passing the input tensor X through the model
                y_hat = self.model(x)
                # Add the mean absolute error from the batch to the sum
                mae += self.model.mean_absolute_error(y_hat, y)
            # Add 1 batch to the count to keep track for averaging
            val_batch_idx += 1
        # Return the mean average of per-batch MAE on the validation dataset
        return mae / val_batch_idx
