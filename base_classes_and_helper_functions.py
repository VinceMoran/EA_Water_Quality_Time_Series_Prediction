...
class TimeSeries(DataModule):
    ...
    def __init__(self,
        ...
        self.y_train_tensor = torch.tensor(y_train, dtype=torch.float32)[num_steps:]
        self.X_val_tensor = torch.tensor(X_val, dtype=torch.float32).unfold(0, num_steps, 1)[:-1].permute(0, 2, 1)
        self.y_val_tensor = torch.tensor(y_val, dtype=torch.float32)[num_steps:]
        self.X_test_tensor = torch.tensor(X_test, dtype=torch.float32).unfold(0, num_steps, 1)[:-1].permute(0, 2, 1)
        self.y_test_tensor = torch.tensor(y_test, dtype=torch.float32)[num_steps:]
